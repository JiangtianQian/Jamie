...
public class QryParser {
    ...
    private static Qry createOperator (String operatorName) {
        ...
        if (operatorNameLowerCase.indexOf('/') > -1) {
            ...
            operatorDistance = Integer.valueOf(tmp[1]);

        }
        //System.out.println("length " + operatorDistance);
        //System.out.println("case " + operatorNameLowerCase);
    ...
    private static Qry parseString (String queryString)
        ...
        while (queryString.length() > 0) {
            ...
            if (queryTree instanceof QrySopWand || queryTree instanceof QrySopWsum) {
                // If the queryTree type needs weight, we add get the weight and add it to
                // weight list.
            ...
            for (int i=0; i<qargs.length; i++) {
                ...
                //If the queryTree type needs weight, we add it to the weight list
                // according to the terms order.
                if (queryTree instanceof QrySopWand || queryTree instanceof QrySopWsum) {
                    weights.add(weight);
                }
        ...
        // If the queryTree type needs weight, we add it to the
        // field of the specific queryTree type, otherwise, just ignore it.
        if (queryTree instanceof QrySopWand || queryTree instanceof QrySopWsum) {
            ((QrySop) queryTree).weight = weights;
        }
        return queryTree;
    }
